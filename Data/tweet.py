...
with open('custom_unprocessed.txt', 'ab') as f:
    for tweet in tweepy.Cursor(api.search, q="#SpiderManHomeComing -filter:retweets", max_id=max_id, since="2017-06-27", until="2017-07-10", lang="en").items():
...
with open("custom_checkpoint.txt", 'ab') as f:
    f.write("%s %s SpiderManHomeComing\n" % (time, str(cp)))
